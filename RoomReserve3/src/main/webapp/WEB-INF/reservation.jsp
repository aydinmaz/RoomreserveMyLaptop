...
<%-- 	    <form:form id = "roomTableForm" method="GET" action="${contextPath}/reservationTest">  --%>
				...
				<tr>
			     ...
			     <c:choose>
			     	...
			     	<c:when test="${m.user.username==currentuser }">
			     	  <td style="background-color: #4fabc9" id="${m.id}" onclick="content(id)">reserved by you ${m.user.name},${mSta.index+1}</td> 
			     	</c:when>
			     <c:otherwise> <td style="background-color:red" id="${m.id}" onclick="content2()">reserved by ${m.user.name},${mSta.index+1}</td> 
			     </c:otherwise>
			     </c:choose>
			    </c:forEach>    
			    </tr>
